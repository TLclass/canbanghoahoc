...
function doBalance() {
    ...
    try {
        const eqn = parse(input);  // Parse the input formula to an equation object
        let matrix = buildMatrix(eqn);  // Set up matrix
        solve(matrix);  // Solve linear system
        const coefs = extractCoefficients(matrix);  // Get coefficients
        
        checkAnswer(eqn, coefs);  // Self-test
        
        const output = eqn.toHtml(coefs);  // Convert to HTML
        document.getElementById('balanced').innerHTML = '';
        document.getElementById('balanced').appendChild(output);
        document.getElementById('message').textContent = 'Successfully balanced!';
